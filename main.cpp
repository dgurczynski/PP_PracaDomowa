...
class TicTacToe {
private:
	static const int CC_BOARD_SIZE = 3;
	char board[CC_BOARD_SIZE][CC_BOARD_SIZE];

public:
	//Funkcja generująca nową planszę
	void init() {
	...
	void move_players(int& turn, bool& time_ended) {
		int x, y;
	...
	void manager(int mode, int& turn) {
		turn = 0;
		...
		while (!game_ended && turn < 9 && !time_ended) {
			display_board();
			if (turn % 2 != 0) { move_players(turn, time_ended); }
			else {
				if (mode == 0) { bot_move(turn); }
				else { move_players(turn, time_ended); }
			}
			to_SVG("kolko_i_krzyzyk.svg");
			game_ended = check_win(winner);
		}
...
class Checkers {
private:
	static const int CHECKERS_BOARD_SIZE = 8;
	char board[CHECKERS_BOARD_SIZE][CHECKERS_BOARD_SIZE];

public:
	void init() {
		char init_board[CHECKERS_BOARD_SIZE][CHECKERS_BOARD_SIZE] = {
			{' ', 'W', ' ', 'W', ' ', 'W', ' ', 'W'},
			{'W', ' ', 'W', ' ', 'W', ' ', 'W', ' '},
			{' ', 'W', ' ', 'W', ' ', 'W', ' ', 'W'},
			{' ', ' ', ' ', ' ', ' ', ' ', ' ', ' '},
	...
	bool if_movement_correct(char player, int x1, int y1, int x2, int y2, bool capture) {
	...
	void move_players(int& turn, char player) {
		int x1, y1, x2, y2;
		char move_start[3], move_end[3];

		bool capture = can_capture(player);
		if (capture) {
		...
		if (x1 >= 0 && x1 < 8 && y1 >= 0 && y1 < 8) {
			if (board[y1][x1] == player) {
				...
				if (x2 >= 0 && x2 < 8 && y2 >= 0 && y2 < 8) {
					...
					if (if_movement_correct(player, x1, y1, x2, y2, capture)) {
						if (abs(y2 - y1) == 2 && abs(x2 - x1) == 2) {
	...
	bool check_win(char& winner, char player) {
		...
		if (can_capture(player)) { return false; }
	...
	void save_game(string file_name, int turn) {
		ofstream file(file_name);
		file << turn << endl;
		for (int i = 0; i < CHECKERS_BOARD_SIZE; i++) {
			for (int j = 0; j < CHECKERS_BOARD_SIZE; j++) {
	...
	void read_game(string file_name, int& turn) {
	...
	void manager(int& turn) {
		...
		while (!game_ended) {
			...
			display_board();
			to_SVG("warcaby.svg");
			move_players(turn, current_player);
			save_game("zapis.txt", turn);
			game_ended = check_win(winner, current_player);
		}
		if (game_ended) {
			cout << "Wygral gracz: " << winner << "!" << endl;
			display_board();
...
public:
	void run() {
		...
		do {
			...
			switch (choice) {
			case '1': {
				svgen.create_SVG("plik_graficzny.svg", 0);
				cout << "Pusty plik zostal wygenerowany." << endl;
				break;
			}

			case '2': {
				svgen.create_SVG("plik_graficzny.svg", 1);
			...
			case '3': {
				ttt.init();
				ttt.manager(1, TTT_turn);
				break;
			}

			case '4': {
				ttt.init();
				ttt.manager(0, TTT_turn);
				break;
			}
